...
Below is code used to publish all the `.md` files in a chosen directory, the `docs` folder in this case. The user needs to add variables (CONFLUENCE_HOST, CONFLUENCE_USERNAME, CONFLUENCE_API_TOKEN and CONFLUENCE_SPACE_KEY) to the `.env` file. When run, gets the credentials and project name to create or get the Confluence folder to publish pages to. Then it finds the `docs` folder and all of the `.md` files contained within. Then, for each found file, Confluence is checked for an existing page. If the page exists it is overwritten, if not, a new page is created.
